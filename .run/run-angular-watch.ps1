...
$restart = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes", "Restart Angular watch"
...
try {
    ...
    do
    {
        Write-Host "`nRunning Angular watch"
        $path = (Get-Command ng).Source
        ...
        Start-Console $path "build --preserve-symlinks --watch" "Angular watch: $ProjectName" -IsClearConsole $false -IsAutoKill $true -KillWaitTimeout 100 -WorkingDirectory $currentDirectory
        ...
        $result = $host.ui.PromptForChoice('Angular watch terminated', 'Do you want to restart the Angular watch?', $options, 0);
    ...
    Write-Error 'Failed to run Angular watch'
